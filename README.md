...
```
Note: do not move or hard link `LTR_classifier.py` alone to anywhere else, as it rely on `database/` and `bin/`. You can add the directory to `PATH` or soft link `LTR_classifier.py` to `PATH`.
...
```
The alignments can also be generated by (have MAFFT installed):
```
python ../bin/concatenate_domains.py rice6.9.5.liban.rexdb.cls.pep RT > rice6.9.5.liban.rexdb.cls.pep.RT.aln
```
The alignments of LTR-RTs full domains can be generated by:
```
...
```
The alignments of Class I INT and Class II TPase (DDE-transposases) can be generated:
```
...
```
Note: the domain names between rexdb and gydb are different: PROT (rexdb) = AP (gydb), RH (rexdb) = RNaseH (gydb).
